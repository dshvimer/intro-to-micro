...
// beginning of area for all students changes
        ...
        {
            if(temp >= temproom * 1.03) {P1OUT |=  BIT5; __delay_cycles(200000);}    // on if dark
            if(temp <= temproom * 1.01) {P1OUT &= ~BIT5; __delay_cycles(200000);}    // off if light
        }

        //Touch Controlling LED3
        //use a dead zone for no action between 0.7-0.9 of the value touch

            if(touch <= touchroom * 0.7) {P2OUT ^= BIT0; __delay_cycles(200000);}
